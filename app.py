...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        bot_msg = TextMessage(text=f"嗨囉你剛才說的是: {user_msg}")

        # 如果使用者傳入的訊息在faq內
        if user_msg in faq:
            #透過key查詢對應的value
            bot_msg = faq[user_msg]
        elif user_msg.lower() in ["menu","選單","主選單","main","首頁","功能表"]:
            bot_msg = menu
        elif user_msg in table:
            #如果使用者輸入的文字是幣別名稱，取的買賣價
...
"""
            ...
            #將文字放入textmessage回應包裝成可回應line server的文字訊息
